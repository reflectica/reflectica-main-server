...
const { auditPHIAccess, auditPHIUpdate } = require('../middleware/auditMiddleware')

route.post("/getAllSessions", auditPHIAccess, async (req, res) => {
  const { userId } = req.body;
  const getAllSessionsForUser = await getAllUserSessions(userId);
  res.send({ sessions: getAllSessionsForUser })
})

route.post("/getSessionTranscripts", auditPHIAccess, async (req, res) => {
...
route.post("/endSession", auditPHIUpdate, async (req, res) => {
